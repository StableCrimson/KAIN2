...
void PHYSOB_PlayDropSound(struct _Instance* instance) // Matching - 100%
{
	...
	if (Prop->family == 1) // need to figure out the family type of 1 for physical objects
	{
		sound = (short)((struct PhysObInteractProperties*)Prop)->auxConditions; // likely wrong
	}
	else if (Prop->family == 0) // need to figure out the family type of 0 for physical objects
	{
		sound = (char)((struct PhysObInteractProperties*)Prop)->razielAuxAnim; // likely wrong
	}
	else if (Prop->family == 3)
	{
		sound = 0;

		if (((struct PhysObInteractProperties*)Prop)->weapon != NULL)
		{
			sound = ((struct PhysObInteractProperties*)Prop)->weapon->dropSound;
		}
	}
	else
	{
...
struct _Instance* PHYSOBS_IsAPushBlockAttached(struct _Instance* block) // Matching - 100%
{
...
int AnyBlocksInMotion() // Matching - 100%
{
	struct _Instance* instance;
	...
	for (; ; )
	{
		...
		if ((instance->object->oflags2 & 0x40000))
		{
			...
			if ((Prop->Type & 8) && (Prop->ID == 0xB00B) && (((struct PhysObData*)(instance->extraData))->Mode & 0x14E))
			{
				return 1;
			}
		}
...
void SetThrowDirection(struct _Instance* instance, struct _Instance* parent, struct evObjectThrowData* throwData, struct PhysObData* Data) // Matching - 100%
{
	long val;
	struct _Instance* itarget;
	MATRIX* matrix;
...
// ************** not on pc
int PushPhysOb(struct _Instance* instance, short x, short y, short PathNumber, struct _Instance* Force) // Matching - 100%
{
	struct PhysObData* Data;
	struct PhysObProperties* Prop;
	int result;
...
// ***** not on pc
void ResetSwitchPhysOb(struct _Instance* instance) // Matching - 100%
{
	struct PhysObData* Data;
	struct PhysObSwitchProperties* Prop;
	struct PhysObSwitchProperties* temp; // not from SYMDUMP

	if (CheckPhysObFamily(instance, 2) != 0)
	{
		Data = (struct PhysObData*)instance->extraData;

		Prop = (struct PhysObSwitchProperties*)instance->data;

		Data->Mode |= 0x800;

		temp = (struct PhysObSwitchProperties*)instance->extraData;

		if ((temp->Properties.ID & 2) && (Prop->shutAnim != 0))
		{
...
// ***** not on pc
int SwitchPhysOb(struct _Instance* instance) // Matching - 100%
{
	struct PhysObData* Data;
	struct PhysObSwitchProperties* Prop;
	...
	temp = (struct PhysObSwitchProperties*)instance->data;
	...
	if (CheckPhysObFamily(instance, 2) != 0)
	{
		...
		Prop = (struct PhysObSwitchProperties*)instance->extraData;

		switchData = (struct SwitchData*)&Prop->Properties.ID;
...
void PhysOb_AlignPush(struct _Instance* instance, int x, int y, int path, struct PhysObData* Data) // Matching - 100%
{
	...
	G2EulerAngles_FromMatrix(&ea, (struct _G2Matrix_Type*)&temp2, 21);
...
int FlipPhysOb(struct _Instance* instance, short x, short y, struct _Instance* Force) // Matching - 100%
{
	struct PhysObData* Data;
	struct PhysObProperties* Prop;
	int result;
	...
	if ((Prop->Type & 2))
	{
		if (PHYSOBS_IsAPushBlockAttached(instance) == 0)
		{
			Data = (struct PhysObData*)instance->extraData;
...
int PickUpPhysOb(struct _Instance* instance, short Steps, struct _Instance* Force, int LinkNode) // Matching - 100%
{
	struct PhysObData* Data;
...
struct _Instance* PHYSOB_BirthCollectible(struct _Instance* parent, int x, int y, int z, int type, int lifeTime) // Matching - 100%
{
	...
	if (type != 0)
	{
		object = (struct Object*)objectAccess[26].object;
	}
	else
	{
		object = (struct Object*)objectAccess[25].object;
	}
	...
	if (object != NULL)
	{
		...
		if (instance != NULL)
		{
			((struct Signal*)(instance->extraData))->data.introFX.time = (lifeTime * 122880);  // double-check cast
...
void PHYSOB_StartLighting(struct _Instance* instance, struct _PhysObLight* pLight) // Matching - 100%
{
	struct PhysObData* Data;
...
void PHYSOB_StartBurnFX(struct _Instance* instance) // Matching - 100%
{
	struct PhysObData* Data;
	struct Object* object;
	int i;
...
void PHYSOB_StopLighting(struct _Instance* instance, struct _PhysObLight* pLight) // Matching - 100%
{
	struct PhysObData* Data;
...
void PHYSOB_StopBurning(struct _Instance* instance, struct _PhysObLight* pLight) // Matching - 100%
{
	struct PhysObData* Data;
...
unsigned long PhysicalObjectQuery(struct _Instance* instance, unsigned long Query) // Matching - 99.94%
{
	struct PhysObData* Data;
	int trueValue;
	...
	case 1:
	{
		struct PhysObAnimatedProperties* temp;  // not from SYMDUMP
		...
		temp = (struct PhysObAnimatedProperties*)instance->data;
	...
	case 5:
	{
		...
		if (CheckPhysObAbility(instance, 0x40) != 0)
		{
			return (short)temp->Properties.ID;
		}
	...
	case 7:
		if (CheckPhysObAbility(instance, 0x40) != 0)
		{
			return SetPositionData(0, 0, ((short)(instance->rotation.z + ((struct PhysObData*)(instance->extraData))->yForce)));
		}
		...
		if (CheckPhysObFamily(instance, 1) != 0)
		{
			...
			{
				G2EulerAngles_FromMatrix(&ea, (struct _G2Matrix_Type*)&instance->matrix[2], 0);
			}
			else
			{
				ea.x = 0;
	...
	case 4:
	{
		...
		if (Prop->family == 3)
		{
			return (short)((struct PhysObInteractProperties*)instance->extraData)->Properties.Type;
		}
		else if (Prop->family == 0)
		{
			return ((struct PhysObWeaponProperties*)Prop)->WeaponAttributes.Class;
		}
		else if (Prop->family == 2)
		{
			return ((struct PhysObSwitchProperties*)Prop)->Class;
		}
	...
	case 22:
	{
		struct PhysObDraftProperties* DraftProp;
		struct INICommand* index;

		if (CheckPhysObAbility(instance, 0x100) != 0)
		{
			index = INSTANCE_FindIntroCommand(instance, 19);

			if (index != NULL)
			{
				DraftProp = (struct PhysObDraftProperties*)instance->data;
			...
			return SetObjectDraftData(DraftProp->force, DraftProp->radius, DraftProp->radiusCoef, (unsigned short)DraftProp->height, DraftProp->maxVelocity);
		}
	...
	case 24:
	{
		...
		struct _PhysObSaveData* data;

		pdata = (struct evControlSaveDataData*)CIRC_Alloc(sizeof(struct evControlSaveDataData) * 2);

		pdata->length = 8;

		pdata->data = pdata + 1;

		memcpy(pdata + 1, (char*)&((struct evControlSaveDataData*)instance->extraData)->data, sizeof(struct SwitchData));

		data = (struct _PhysObSaveData*)pdata + 1;
	...
	case 47:
	{
		struct PhysObData* physobData;

		physobData = (struct PhysObData*)instance->extraData;
...
long PhysobAnimCallback(struct _G2Anim_Type* anim, int sectionID, enum _G2AnimCallbackMsg_Enum message, long messageDataA, long messageDataB, void* data) // Matching - 100%
{
	struct PhysObData* Data;

	if (message == 1)
	{
		Data = (struct PhysObData*)((struct _Instance*)data)->extraData;
		...
		{
			INSTANCE_KillInstance((struct _Instance*)data);
		}

		return messageDataA;
	}

	return INSTANCE_DefaultAnimCallback(anim, sectionID, message, messageDataA, messageDataB, (struct _Instance*)data);
}
...
struct PhysObWeaponAttributes* PhysObGetWeapon(struct _Instance* instance) // Matching - 100%
{
	struct PhysObWeaponAttributes* weapon;
	struct evObjectBirthProjectileData* temp; // not from SYMDUMP
	...
	if (CheckPhysObFamily(instance, 3) != 0)
	{
		...
		Prop = (struct PhysObInteractProperties*)instance->data;
	...
	if (CheckPhysObFamily(instance, 7) != 0)
	{
		...
		Prop = (struct __PhysObProjectileProperties*)instance->data;

		temp = (struct evObjectBirthProjectileData*)instance->extraData;

		weapon = Prop->data[temp->joint].weapon;
	}
...
void TurnOnCollisionPhysOb(struct _Instance* instance, int coll) // Matching - 100%
{
	struct PhysObWeaponAttributes* weapon;

	((struct PhysObData*)(instance->extraData))->Mode &= ~0x1000000;
...
int GetPhysObCollisionType(struct _Instance* instance) // Matching - 99.95%
{
	...
	{
		weapon = (struct PhysObWeaponAttributes*)collideInfo->prim0;
		...
		if (((collideInfo->type0 == 1 && collideInfo->type1 == 1)) && ((unsigned char)weapon->AltDamage == 9) && (S = (struct _HSphere*)collideInfo->prim1, S->id == 8))
		{
			...
			if (weapon != NULL)
			{
				...
				if (instance->LinkParent == NULL)
				{
					if ((INSTANCE_Query(target, 0) & 0x10000000))
					{
						if (weapon->Class == 1)
						{
							INSTANCE_Post(target, 0x1000019, (int)instance);
						...
						else
						{
							...
							{
								INSTANCE_Post(target, 0x100000C, 32);
					...
					{
						INSTANCE_Post(target, 0x80001, NULL);
					}
				}
				else
				{
...
long PHYSOBS_CheckForStackedForwardHits(struct _Instance* block, long xoffset, long yoffset) // Matching - 100%
{
	struct _Instance* attachedBlock;
	long result;
	...
	{
		if (PHYSOB_CheckDirectedLineCollision(attachedBlock, xoffset, yoffset, 0))
		{
			result = 1;
			break;
		}
...
long PHYSOB_CheckForEnemyInBlkSpot(struct _Instance *instance, int dx, int dy) // Matching 100%
{
    ...
    while (inst != NULL) 
	{
        ...
        if (!(inst->object->oflags2 & 0x80000) || 
            inst->flags2 & 0x08000000 || 
            INSTANCE_Query(inst, 0) & 0x40000000 || 
            inst->position.x < x0 || 
            x1 < inst->position.x || 
            inst->position.y < y0 || 
            y1 < inst->position.y || 
